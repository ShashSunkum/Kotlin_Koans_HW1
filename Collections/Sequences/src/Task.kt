...
fun findMostExpensiveProductBy(customer: Customer): Product? {
    return customer.orders
        .filter { it.isDelivered }  // Filter only delivered orders
...
fun Customer.getOrderedProducts(): Sequence<Product> =
    orders.asSequence().flatMap { it.products.asSequence() }  // Convert orders to Sequence and flatten the products
